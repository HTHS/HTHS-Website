<div id="content_left">
	<div id="content_left_above">
		<div class="fancybox">
			...
			<? endif; ?>
			<ul>
				<? foreach($students->result() as $user): ?>
					...
					<div id="hide<?=$user->id?>" style="display:none;" class="fancybox">
						...
						<input type="button" value="View Log" onclick="window.location.href = '<?=site_url('teachers/dashboard/mentorship/view/'.$user->id)?>'" /> <input type="button" value="Edit Student" onclick="window.location.href = '<?=site_url('teachers/dashboard/mentorship/edit_student/'.$user->id)?>'" /> <input type="button" value="Reset Password" onclick="window.location.href = '<?=site_url('teachers/dashboard/mentorship/reset_password/'.$user->id)?>'" /> <input type="button" value="Delete Student" onclick="var c = confirm('This will remove the student <?=$user->name?> and all their logs from the system permenantly! Are you absolutely sure you want to proceed?'); if(c) { window.location.href = '<?=site_url('teachers/dashboard/mentorship/delete_student/'.$user->id)?>' }" />
					</div>
				<? endforeach; ?>
			</ul>
		</div>
