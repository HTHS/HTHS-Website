<div id="content_left">
	<div id="content_left_above">
		<div class="fancybox">
			...
			<?php else: ?>
			<ul>
				<?php foreach($results->result() as $user): ?>
					...
					<div id="hide<?=$user->id?>" style="display:none;" class="fancybox">
						...
						<input type="button" value="View Log" onclick="window.location.href = '<?=site_url('mentorship_admin/view/'.$user->id)?>'" /> <input type="button" value="View Timesheets" onclick="window.location.href = '<?=site_url('mentorship_admin/view_times/'.$user->id)?>'" /><br /><input type="button" value="Edit Student" onclick="window.location.href = '<?=site_url('mentorship_admin/edit_student/'.$user->id)?>'" /> <input type="button" value="Delete Student" onclick="var c = confirm('This will remove the student <?=$user->name?> and all their logs from the system permenantly! Are you absolutely sure you want to proceed?'); if(c) { window.location.href = '<?=site_url('mentorship_admin/delete_student/'.$user->id)?>' }" />
					</div>
				<?php endforeach; ?>
			</ul>
			<?php endif; ?>
