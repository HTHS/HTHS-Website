				...
				<? foreach($teachers->result() as $teacher): 
					...
					<form action="<?=site_url('admin/edit_teacher/'.$teacher->id)?>" method="post">
						<table>
							...
							<tr>
								<td colspan="2"><input type="submit" value="Edit Teacher" /> <input type="button" value="Delete Teacher" onclick="window.location.href = '<?=site_url('admin/delete_teacher/'.$teacher->id)?>'" /> <input type="button" value="Cancel" id="cancel<?=$teacher->id?>" /></td>
							</tr>
						</table>
					</form>
					</div>
